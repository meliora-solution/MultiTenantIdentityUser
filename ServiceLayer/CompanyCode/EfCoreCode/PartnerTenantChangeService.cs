...
namespace PartnerCode.EfCoreCode
{
    public class PartnerTenantChangeService : ITenantChangeService
    {
    ...
    //    private readonly ILogger _logger;
        ...
        public PartnerTenantChangeService(PartnerDbContext context)
        {
            _context = context;
          //  _logger = logger;
        }
        //public PartnerTenantChangeService(PartnerDbContext context, ILogger<PartnerTenantChangeService> logger)
        //{
        //    _context = context;
        //    _logger = logger;
        //}
        ...
        public async Task<string> SingleTenantDeleteAsync(Tenant tenant)
        {
            ...
            try
            {
                ...
                /*
                   var deleteSalesSql = $"DELETE FROM invoice.{nameof(PartnerDbContext.LineItems)} WHERE DataKey = '{dataKey}'";
                await _context.Database.ExecuteSqlRawAsync(deleteSalesSql);
                var deleteStockSql = $"DELETE FROM invoice.{nameof(PartnerDbContext.Invoices)} WHERE DataKey = '{dataKey}'";
